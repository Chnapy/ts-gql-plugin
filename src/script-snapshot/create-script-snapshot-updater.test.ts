...
describe('Create script snapshot updater', () => {
  ...
  \``;
    ...
    const typeStatement1 =
      '` as TypedDocumentNode<UserQueryOperation, UserQueryVariables>';
    const typeStatement2 =
      '` as TypedDocumentNode<TotoQueryOperation, TotoQueryVariables>';
    ...
    export const CartList: React.FC = () => {
      const { t } = useTranslate();
      const { data: cartData, loading } = useQuery(${query1.slice(
        0,
        query1.length - typeStatement1.length
      )}${typeStatement1});
      const { data: totoData } = useQuery(${query2.slice(
        0,
        query2.length - typeStatement2.length
      )}${typeStatement2});
      const cartItems = useGraphQLArray(cartData?.cartItems);
      const [removedVendorProductId, setRemovedVendorProductId] = React.useState<string | null>(null);
      const cartPrice = useCartPrice();
